...
const { fetchGameState } = require("./gameState");
...
const calcAttackBuff = async (connection, id, nftPubkey, state = null) => {
    if (!state) { state = await fetchGameState(id); }
    const [trifleAddress] = findTriflePda(nftPubkey, new PublicKey(state.trifleAuthority));;
    const trifleTokens = await getTrifleTokens(connection, trifleAddress);
    const buffs = calcBuffs(state, trifleTokens);
    return buffs.attack;
}

const calcBuffs = (state, trifledTokens) => {
    let attack = 0, haste = 0;
    const {items} = state;
    items.forEach(i => {
        if (Array.isArray(trifledTokens.attack)) {
            trifledTokens.attack.forEach(a => {
                if (i.address === a.mint.toString()) {
                    attack += i.value;
                }
            });
        }
        if (Array.isArray(trifledTokens.haste)) {
            trifledTokens.haste.forEach(a => {
                if (i.address === a.mint.toString()) {
                    haste += i.value;
                }
            });
...
// for debugging
const showModel = async (connection, modelAddress, onlyConstraints = false) => {
    const accountInfo = await connection.getAccountInfo(modelAddress);
    if (accountInfo) {
      const account = EscrowConstraintModel.fromAccountInfo(accountInfo)[0];
      account.constraints = Object.fromEntries(account.constraints);
      if (onlyConstraints) {
        console.log(account.constraints);
      } else {
        console.log(account);
      }
    } else {
